...
#              1.5.4
#              (1) Add easy evaluation mode for single reference evaluations with -z
#                  option.
#              1.5.3
...
#                  improve the reliability of the final score that treats each token as
...
#                  be controlled by 'alpha' parameter:
...
#                  This is seen in evaluation where ROUGE-L score is higher than ROUGE-1 and
#                  this is not correct.
...
#              1.2.3
#              (1) Correct the environment variable option: -e. Now users can specify environment
...
" .
    ...
    "      0: use sentence as counting unit, 1: use token as counting unit, 2: same as 1 but output raw counts\n" .
...
sub computeAverages {
    ...
    if (!defined($opt_t) || $opt_t == 1) {
        ...
        # find confidence intervals; take maximum distance from the mean
...
sub computeROUGEX {
    ...
    my $lengthLimit;        # length l  imit in words
...
sub computeNGramScore {
    ...
    foreach $modelPath (@$modelPaths) {
        ...
        # credit to redundant information contained in the peer summary.
        if ($scoreMode eq "A") {
...
sub computeLCSScore {
    ...
    foreach $modelPath (@$modelPaths) {
        %tmp_peer_1grams = %peer_1grams; # renew peer unigram hash, so the peer count can be reset to the original number
        @modelTokens = ();
        @modelText = ();
        ...
        for ($j = 0; $j < @{$model->[$i]}; $j++) {
            if ($hitMask[$j] == 1) {
                if (exists($model_1grams->{$model->[$i][$j]}) &&
                    ...
                    # every time a hit is found it is deducted
    ...
    for ($i = 1; $i <= $m; $i++) {
        for ($j = 1; $j <= $n; $j++) {
            if ($model->[$i - 1] eq $peer->[$j - 1]) {
                # recursively solve the i-1 sub problem
                $c[$i][$j] = $c[$i - 1][$j - 1] + 1;
                $b[$i][$j] = "\\"; # go diagonal
            }
...
sub stem {
    ...
    if ($w =~ /e$/) {
        $stem = $`;
        if ($stem =~ /$mgr1/o or
            ($stem =~ /$meq1/o and not $stem =~ /^${C}${v}[^aeiouwxy]$/o)) {$w = $stem;}
    }
