...
from rouge.tests.wrapper import rouge_s_sentence_level as _rouge_s_sentence_level
from rouge.tests.wrapper import rouge_s_summary_level as _rouge_s_summary_level
...
class TestRougeS(unittest.TestCase):
    ...
    def test_example(self):
        ours_score = rouge_s_sentence_level(summary, reference)
        theirs_score = _rouge_s_sentence_level(summary, reference)
    ...
    def test_sentence_level(self):
        for ours_data, theirs_data in load_sentence_pairs():
            ours_score = rouge_s_sentence_level(*ours_data)
            theirs_score = _rouge_s_sentence_level(*theirs_data)
    ...
    def test_summary_level(self):
        for ours_data, theirs_data in load_summary_pairs():
            ours_score = rouge_s_summary_level(*ours_data)
            theirs_score = _rouge_s_summary_level(*theirs_data)
